...
func TestCanGetNoteIndex(t *testing.T) {
	...
	code, body, err := helpers.HttpRequestForTest(app, "GET", "/api/v1/notes", testArg{"10", "1", "ID asc"}, "")
	assert.Nil(t, err)
...
func TestCanGetNoteDetail(t *testing.T) {
	...
	code, body, err := helpers.HttpRequestForTest(app, "GET", "/api/v1/notes/2", nil, "")
	assert.Nil(t, err)
	assert.Equal(t, 200, code)
	assert.NotEmpty(t, body["data"])
}
...
func TestCanPostNewNoteUsingProperPayload(t *testing.T) {
	...
	code, body, err := helpers.HttpRequestForTest(app, "POST", "/api/v1/notes", nil, payload)
	assert.Nil(t, err)
	assert.Equal(t, 200, code)
	assert.NotEmpty(t, body["data"])
}
